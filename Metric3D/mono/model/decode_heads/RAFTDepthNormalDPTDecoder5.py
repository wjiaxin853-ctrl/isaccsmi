...
class RAFTDepthNormalDPT5(nn.Module):
    """
    RAFTDepthNormalDPT5
    该类将 ViT 编码后的 Token 特征转换为多尺度卷积特征，并通过:
      1) 初始深度+法线回归 (基于离散深度锚点的 softmax 期望)
      2) 迭代式 GRU 更新 (RAFT 风格多层 ConvGRU，逐步细化预测)
      3) 法线置信度与深度置信度联合输出
    架构核心组件:
      - EncoderFeature(token2feature): 将四层 ViT Token (含 cls/register token) 转成多尺度特征 (1/14,1/14,1/7,1/4)
      - DecoderFeature(decoder_mono): 类 DPT 的级联上采样与融合，得到用于深度/法线初始估计的特征
      - depth_regressor: 离散深度分布 (num_depth_regressor_anchor 个 bin)，通过 softmax+期望得到连续深度 (再线性尺度回归与 clamp)
      - normal_predictor: 基于共享特征的法线 MLP 预测 (输出 3 通道再配合置信度拼接，归一化到单位球面)
      - context_feature_encoder: 生成多层 GRU 的初始隐状态与上下文特征 (x4,x8,x16 尺度)
      - update_block(BasicMultiUpdateBlock): 包含多尺度 ConvGRU；输入当前 flow(深度+置信度+法线打包为“流”场)与上下文特征递归更新
    主要数据流:
      ViT 特征 -> token2feature -> 多尺度特征 -> decoder_mono -> 初始深度/法线 + 置信度 -> 构造初始 flow 张量
      -> (多次迭代) ConvGRU 更新 -> 上采样 -> clamp/归一化 -> 最终输出字典
    深度数值范围:
      - 原始网络输出为经过回归缩放 (regress_scale) 的值再通过 clamp 限制在 [min_val, max_val]
      - num_depth_regressor_anchor 定义离散锚点数量 (log 均匀间隔)
    迭代逻辑:
      - iters 次循环，每次用 update_block 计算 delta_flow (包含对深度/法线等的残差更新)
      - coords1 = coords1 + delta_flow 累积
      - 最终 upsample_flow/upsample 得到高分辨率预测
    输出字典字段:
      prediction: 最终深度 (B,1,H,W)
      predictions_list: 每次迭代的深度列表
      confidence: 最终深度置信度 (B,1,H,W)
      confidence_list: 迭代置信度列表
      prediction_normal: 最终法线 (B,4,H,W) 其中前三通道为归一化法线, 第四通道为 kappa/置信度
      normal_out_list: 迭代法线列表
      low_resolution_init: 初始低分辨率的 [深度, 深度置信度, 法线] 用于分析或可视化
    适用场景:
      - 单目深度 + 法线联合估计
      - 需要迭代细化和不确定性表达的几何预测任务
    """
    def __init__(self, cfg):
        super().__init__()
        self.in_channels = cfg.model.decode_head.in_channels  # ViT 输出 token 通道数 (每层相同)
        self.feature_channels = cfg.model.decode_head.feature_channels  # 多尺度特征通道 (token2feature 输出)
        self.decoder_channels = cfg.model.decode_head.decoder_channels  # 解码器级联各阶段通道
        self.use_cls_token = cfg.model.decode_head.use_cls_token  # 是否使用 cls + register token
        self.up_scale = cfg.model.decode_head.up_scale  # 最终上采样尺度基准
        self.num_register_tokens = cfg.model.decode_head.num_register_tokens  # register token 数量
        self.min_val = cfg.data_basic.depth_normalize[0]  # 深度 clamp 下界
        self.max_val = cfg.data_basic.depth_normalize[1]  # 深度 clamp 上界
        self.regress_scale = 100.0  # 回归尺度因子 (反缩放时乘回并 clamp)

        self.hidden_dims = self.context_dims = cfg.model.decode_head.hidden_channels  # GRU 隐状态与上下文维度
        self.n_gru_layers = cfg.model.decode_head.n_gru_layers  # GRU 层数 (多尺度层数)
        self.n_downsample = cfg.model.decode_head.n_downsample  # 下采样因子指数 (用于掩膜上采样)
        self.iters = cfg.model.decode_head.iters  # 迭代次数
        self.slow_fast_gru = cfg.model.decode_head.slow_fast_gru  # 是否使用慢-快交替更新策略

        self.num_depth_regressor_anchor = 256  # 深度离散锚点数量 (log space)
        self.used_res_channel = self.decoder_channels[1]  # 使用解码器中某一尺度通道作为共享特征
        ...
        # 上下文特征编码 (为 GRU 初始化与后续融合提供特征)
        self.context_feature_encoder = ContextFeatureEncoder(self.feature_channels, [self.hidden_dims, self.context_dims])
        # 将上下文特征经一次卷积拆分为 z,r,q 三部分 (每层 GRU 使用)
        self.context_zqr_convs = nn.ModuleList([
            nn.Conv2d(self.context_dims[i], self.hidden_dims[i]*3, 3, padding=1) for i in range(self.n_gru_layers)
        ])
        # 多尺度更新模块 (核心迭代逻辑)
    ...
    def forward(self, vit_features, **kwargs):
        """
        前向步骤:
          1. 解析 ViT features (拆分 patch token 与 cls/register token)
          2. token2feature -> 多尺度卷积特征
          3. decoder_mono -> 初始共享特征 (深度/法线回归输入)
          4. regress_depth + pred_normal -> 初始深度/法线 + 置信度
          5. 构造初始 flow (coords1 = coords0 + depth_init)
          6. iters 次迭代: ConvGRU 递归更新 delta_flow
          7. 上采样 (upsample_flow 或 nearest) 得到最终高分辨率预测
          8. 组装输出字典
        输出 keys 详见类 docstring。
        """
        ## 1. 解析 ViT features
        # 获取批量大小 B、高度 H、宽度 W、以及 register token 数量
        B, H, W, _, _, num_register_tokens = vit_features[1]
        vit_features = vit_features[0]

        # 检查 ViT 特征是否存在 NaN 或 Inf
        if torch.isnan(vit_features[0]).any():
            print('vit_feature_nan!!!')
        if torch.isinf(vit_features[0]).any():
            print('vit_feature_inf!!!')

        # 根据是否使用 cls token，调整 ViT 特征的形状
        ...
        # 2. ViT 特征转换为多尺度卷积特征
        encoder_features = self.token2feature(vit_features)  # 输出 1/14, 1/14, 1/7, 1/4 尺度特征

        # 检查编码器特征是否存在 NaN 或 Inf
        ...
        ## 3. 解码器生成初始深度和法线特征
        ref_feat = self.decoder_mono(encoder_features)  # 输出 1/4 尺度特征

        # 检查解码器输出是否存在 NaN 或 Inf
        ...
        # 提取共享特征、深度置信度和法线置信度
        feature_map = ref_feat[:, :-2, :, :]  # 深度和法线共享的特征
        depth_confidence_map = ref_feat[:, -2:-1, :, :]
        normal_confidence_map = ref_feat[:, -1:, :, :]

        # 4. 回归深度和预测法线
        depth_pred, binmap = self.regress_depth(feature_map)  # 深度回归
        normal_pred = self.pred_normal(feature_map, normal_confidence_map)  # 法线预测

        # 拼接初始深度、深度置信度和法线
        depth_init = torch.cat((depth_pred, depth_confidence_map, normal_pred), dim=1)  # (N, 1+1+4, H, W)

        ## 5. 编码器特征生成 GRU 的初始隐状态和上下文特征
        cnet_list = self.context_feature_encoder(encoder_features[::-1])  # 倒序输入
        net_list = [torch.tanh(x[0]) for x in cnet_list]  # GRU 隐状态 (x_4, x_8, x_16)
        inp_list = [torch.relu(x[1]) for x in cnet_list]  # 上下文特征 (x_4, x_8, x_16)
        ...
        ## 7. 组装输出字典
        outputs = dict(
            prediction=flow_predictions[-1],  # 最终深度预测
            predictions_list=flow_predictions,  # 每次迭代的深度预测
            confidence=conf_predictions[-1],  # 最终深度置信度
            confidence_list=conf_predictions,  # 每次迭代的深度置信度
            pred_logit=None,
            prediction_normal=normal_outs[-1],  # 最终法线预测
            normal_out_list=normal_outs,  # 每次迭代的法线预测
            low_resolution_init=low_resolution_init,  # 初始低分辨率预测
        )
