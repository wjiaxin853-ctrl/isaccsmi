...
class VNLoss(nn.Module):
    ...
    def select_points_groups(self, gt_depth, pred_depth, intrinsic, mask):
        ...
        # robust guard: avoid invalid indexing / empty selection on custom datasets
        valid_num = int(mask_broadcast.sum().item())
        if valid_num <= 0:
            empty = pw_groups_pred.new_zeros((1, 0, 3, 3))
            return empty, empty
        ...
        try:
            ...
            # Fallback for potential device-side indexing assert
            empty = pw_groups_pred.new_zeros((1, 0, 3, 3))
            return empty, empty
    ...
    def get_loss(self, prediction, target, mask, intrinsic, select=True, **kwargs):
        ...
        # no valid triplets for current batch -> return zero loss safely
        if gt_points.shape[1] == 0 or pred_points.shape[1] == 0:
